...
const {
  validateRegister,
  validateLogin,
  validateMenuItem: _validateMenuItem,
  validateOrder: _validateOrder,
  validateOrderId: _validateOrderId,
  validateObjectId: _validateObjectId,
  validateStatusUpdate: _validateStatusUpdate,
  validateLocation: _validateLocation,
  validateCreateUser: _validateCreateUser,
  validatePasswordChange: _validatePasswordChange
...
// Import location security middleware
const {
...
// Update delivery person location with enhanced security
app.post('/api/delivery/update-location',
  authenticateToken,
  ...
  async (req, res) => {
    try {
      ...
      if (!order.assignedTo || order.assignedTo.toString() !== req.user.id) {
        console.warn(`[SECURITY] Unauthorized location update attempt by user ${req.user.id} for order ${orderId}`)
      ...
      // Check if order status allows location updates
      if (!['out_for_delivery', 'ready'].includes(order.status)) {
        return res.status(400).json({
          error: 'Cannot update location for this order status',
      ...
      if (req.suspiciousLocation) {
        logger.warn('Suspicious location update', {
          userId: req.user.id,
          orderId,
          coordinates: { latitude, longitude },
          flag: 'Abnormal speed detected'
        })
      }
      ...
      res.json({
        success: true,
        tracking,
        warning: req.suspiciousLocation ? 'Location update recorded but flagged for review' : undefined
      })
    } catch (err) {
...
app.get('/api/delivery/track/:orderId',
  authenticateToken,
  trackingViewLimiter,
  async (req, res) => {
    try {
      ...
      // Check authorization - only order owner, assigned delivery person, or managers can track
      const isOrderOwner = order.userId.toString() === req.user.id
      const isAssignedDelivery = order.assignedTo && order.assignedTo._id.toString() === req.user.id
      const isManager = ['admin', 'superadmin', 'ordermanager'].includes(req.user.role)

      if (!isOrderOwner && !isAssignedDelivery && !isManager) {
        console.warn(`[SECURITY] Unauthorized tracking attempt by user ${req.user.id} for order ${orderId}`)
        return res.status(403).json({
      ...
      // Filter location data based on order status and privacy
      let filteredTracking = null
      if (tracking && order.status === 'out_for_delivery') {
        filteredTracking = {
          currentLocation: tracking.currentLocation,
          lastUpdate: tracking.lastUpdate,
          deliveryPersonId: tracking.deliveryPersonId,
          // Don't expose full location history to customers for privacy
          locationHistory: isManager ? tracking.locationHistory : undefined
        }
      } else if (tracking && order.status !== 'out_for_delivery') {
      // Don't expose location if not actively delivering
        filteredTracking = {
          status: 'Location tracking not active for current order status',
      ...
      res.json({
        order: {
          ...
          status: order.status,
          deliveryAddress: isOrderOwner || isAssignedDelivery || isManager ? order.deliveryAddress : undefined,
